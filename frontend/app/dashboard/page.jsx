...
export default function DashboardPage() {
  ...
  const statusColor = (status) =>
    status === 'sent' ? 'text-green-600 dark:text-green-400' : 'text-yellow-500 dark:text-yellow-400';
  ...
  if (status === 'loading' || loading) return <p className="p-4 text-gray-700 dark:text-gray-300">Loading...</p>;
  if (!session) return null;

  return (
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
        {[
          ...
          <div
            key={i}
            className="bg-white dark:bg-gray-800 hover:shadow-lg transition rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700 flex flex-col items-start"
          >
            <h2 className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{card.title}</h2>
            <p className="text-2xl sm:text-4xl font-bold text-blue-600 dark:text-blue-400">{card.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 shadow rounded-xl p-4 sm:p-6">
        <h2 className="text-base sm:text-lg font-semibold mb-2 sm:mb-4 text-gray-900 dark:text-gray-100">📈 Data Overview</h2>
      ...
      {/* Recent Campaigns */}
      <div className="bg-white dark:bg-gray-800 shadow rounded-xl p-4 sm:p-6">
        <h2 className="text-base sm:text-lg font-semibold mb-2 sm:mb-4 text-gray-900 dark:text-gray-100">🕒 Recent Campaigns</h2>
        {recentCampaigns.length === 0 ? (
          ...
          <ul className="space-y-3 sm:space-y-4">
            {recentCampaigns.map((camp) => (
              ...
              >
                <div>
                  <p className="text-md font-semibold text-gray-800 dark:text-gray-100">{camp.name}</p>
